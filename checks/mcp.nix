# MCP 域(构建期):secret 注入行为级(真跑 wrapper 启动块)、stderr
# 收纳行为级、insert 通道端到端(真 boot 进树)。
# 纯求值断言(行渲染金样/refs 收集/名校验)在 tests/mcp.nix
{
  pkgs,
  dshLib,
  fx,
...
in
{
  ...
  dsh-mcp-stderr-log =
    let
      ...
      run = pkgs.lib.concatStringsSep " " ([ fs.command ] ++ map pkgs.lib.escapeShellArg fs.args);
    in
